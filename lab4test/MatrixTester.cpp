...
void MatrixTester::testCopyConstructor() {
	...
	// copy empty matrix
	Matrix m1;
	Matrix m2(m1); // @suppress("Invalid arguments")
	assert( m2.getRows() == 0 );
	assert( m2.getColumns() == 0 );
	...
	Matrix m4(m3); // @suppress("Invalid arguments")
...
void MatrixTester::testSubscripts() {
	...
	testReadSubscript(m); // @suppress("Invalid arguments")
	cout << " 2 " << flush;
...
void MatrixTester::testWriteToFile() {
	...
	// now, read what we just wrote into a different Matrix
	//Matrix m1;
		ifstream bin("MatrixTest1Copy2.txt");
		assert( bin.is_open() );
		unsigned bows, bolumns;
		fin >> bows >> bolumns;
		Matrix m1(bows, bolumns);
		m1.readFrom(bin);
	// test it
	assert( m1.getRows() == 3 );
	assert( m1.getColumns() == 4 );
