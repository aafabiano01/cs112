...
void AccountManager::changeUserName(const string& oldUserName, const string& newUserName) {
	...
	if (myAccounts.find(oldUserName) != myAccounts.end()) {
		it = myAccounts.find(oldUserName);
		string password = it->second;
		try {
			myAccounts.erase(it);
			createNewAccount(newUserName, password);
		} catch (Exception e) {
			throw Exception("changeUserName()", "account dne");
		}
	} else {
		throw Exception("changeUserName()", "account dne");
	}
...
void AccountManager::deleteAccount(const string& userName) {
	if (myAccounts.find(userName) != myAccounts.end()) {
		...
		throw Exception("delete()", "account dne");
	}
}
...
void AccountManager::changePassword(const string& userName,
	...
	map<string, string>::iterator it;
	if (myAccounts.find(userName) != myAccounts.end()) {
		it = myAccounts.find(userName);
		it->second = newPassword;
	} else {
		throw Exception("changePassword()", "account dne");
	}
}
